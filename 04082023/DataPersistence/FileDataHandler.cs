...
public class FileDataHandler
{
    ...
    public GameData Load(string profileId, bool allowRestoreFromBackup = true) 
    {
        ...
        if (profileId == null) 
        {
            Debug.Log("profile id is null, returning from load..");
            return null;
        }

        // use Path.Combine to account for different OS's having different path separators
        string userpath = PlayerPrefs.GetString("Username", "defaultUsername");
        string fullPath = Path.Combine(dataDirPath, userpath, profileId, dataFileName);
    ...
    public void Save(GameData data, string profileId) 
    {
        ...
        if (profileId == null) 
        {
            Debug.Log("profile id is null, returning from save..");
            return;
        }
    ...
    public Dictionary<string, GameData> LoadAllProfiles() 
    {
        ...
        string userPath = PlayerPrefs.GetString("Username", "defaultUsername");
        string userDirPath = Path.Combine(dataDirPath, userPath);

        // loop over all directory names in the data directory path
        // IEnumerable<DirectoryInfo> dirInfos = new DirectoryInfo(dataDirPath).EnumerateDirectories();
        IEnumerable<DirectoryInfo> dirInfos = new DirectoryInfo(userDirPath).EnumerateDirectories();
        foreach (DirectoryInfo dirInfo in dirInfos) 
        {
            ...
            string fullPath = Path.Combine(userDirPath, profileId, dataFileName);
            if (!File.Exists(fullPath))
            {
